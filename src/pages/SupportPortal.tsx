...
export function ExamArchive({ auth = loadCloudflareConfig(), owner = true }: { auth?: Auth; owner?: boolean }) {
  ...
  const remove = async (doc: Document) => {
    if (!window.confirm(`“${doc.title}” PDF를 삭제할까요? 이 작업은 되돌릴 수 없습니다.`)) return;
    setBusy(true); setError('');
    try { await api(auth, `/api/exams/${doc.id}`, 'DELETE'); setPdf(''); await refresh(); }
    catch (e) { setError(errorText(e)); } finally { setBusy(false); }
  };
