...
export default function InsightsHub({ data, update, view, onView }: { data: AppData; update: (fn: (value: AppData) => AppData) => void; view: InsightsView; onView: (view: InsightsView) => void }) {
  ...
  if (view === 'performance') return <><SegmentedControl label="Insights 화면" options={tabs} value={view} onChange={onView} /><Statistics data={data} /></>;
  if (view === 'review') return <><SegmentedControl label="Insights 화면" options={tabs} value={view} onChange={onView} /><PlaireReview data={data} update={update} /></>;
  if (view === 'bottlenecks') return <div><SegmentedControl label="Insights 화면" options={tabs} value={view} onChange={onView} /><PageHeader eyebrow="DIAGNOSE" title="병목과 재현" description="빈도만 보지 않고, 교정 행동이 재도전과 전이로 이어졌는지 확인합니다." />
