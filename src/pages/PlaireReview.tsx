...
import TrinityAnalysis from './TrinityAnalysis';
...
export default function PlaireReview({ data, update }: { data: AppData; update: (fn: (value: AppData) => AppData) => void }) {
  const [date,setDate]=useState(toDateKey()); const [entry,setEntry]=useState(data.plaire[date]??blank(date)); const [copied,setCopied]=useState(false); const [view,setView]=useState<'journal'|'plaire'|'trinity'>('journal');
  ...
  return <div><PageHeader eyebrow="REVIEW HUB" title="Journal · Plaire · Trinity" description="일일 관찰, 학습 프레임 리뷰, 과목별 판단 분석을 한 곳에서 관리합니다." action={view==='plaire'?<input className="date-input" type="date" value={date} onChange={(e)=>setDate(e.target.value)}/>:undefined} />
    <div className="review-tabs triple"><button className={view==='journal'?'active':''} onClick={()=>setView('journal')}>JOURNAL</button><button className={view==='plaire'?'active':''} onClick={()=>setView('plaire')}>PLAIRE</button><button className={view==='trinity'?'active':''} onClick={()=>setView('trinity')}>TRINITY</button></div>
    {view==='journal'?<Journal data={data} update={update} embedded/>:view==='trinity'?<TrinityAnalysis data={data} update={update} embedded/>:<><Card className="plaire-record"><Field label="오늘 학습 기록"><TextArea value={entry.record} onChange={(v)=>set('record',v)} placeholder="과목, 범위, 수행량, 실전 조건"/></Field></Card>
