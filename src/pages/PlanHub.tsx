...
export default function PlanHub({ data, update, view, onView }: { data: AppData; update: (fn: (value: AppData) => AppData) => void; view: PlanView; onView: (view: PlanView) => void }) {
  if (view === 'calendar') return <><SegmentedControl label="Plan 화면" options={tabs} value={view} onChange={onView} /><CalendarPage data={data} update={update} /></>;
  if (view === 'weekly') return <><SegmentedControl label="Plan 화면" options={tabs} value={view} onChange={onView} /><PlanningPage data={data} update={update} /></>;
  if (view === 'routine') return <><SegmentedControl label="Plan 화면" options={tabs} value={view} onChange={onView} /><Routine data={data} update={update} /></>;
  const today = toDateKey(); const week = weekStartKey();
  const plans = Object.values(data.calendar).filter((entry) => entry.date >= week).flatMap((entry) => entry.plans ?? []);
  const todayPlans = data.calendar[today]?.plans ?? [];
  const goals = data.weeklyCapabilityGoals.filter((goal) => goal.weekStart === week);
  return <div><SegmentedControl label="Plan 화면" options={tabs} value={view} onChange={onView} /><PageHeader eyebrow="PLAN" title="실행을 설계합니다" description="시간 계획과 이번 주에 개선할 능력을 같은 흐름에서 정리합니다." />
    ...
    <div className="hub-two-column"><Card><div className="hub-card-head"><div><span className="card-label">NEXT</span><h2>오늘의 계획</h2></div><button className="text-button" onClick={() => onView('calendar')}>Calendar 열기</button></div>{todayPlans.length ? <div className="compact-list">{todayPlans.slice(0, 5).map((item) => <div key={item.id}><ListTodo /><span><b>{item.title}</b><small>{item.subject} · {item.quantity || '목표량 없음'}</small></span><em>{item.done ? '완료' : '예정'}</em></div>)}</div> : <Empty>오늘 계획이 없습니다.<br />Calendar에서 첫 일정을 추가하세요.</Empty>}</Card><Card><div className="hub-card-head"><div><span className="card-label">CAPABILITY</span><h2>이번 주 능력 목표</h2></div><button className="text-button" onClick={() => onView('weekly')}>목표 관리</button></div>{goals.length ? <div className="compact-list">{goals.map((goal) => <div key={goal.id}><CheckCircle2 /><span><b>{goal.ability}</b><small>{goal.successCriterion}</small></span><em>{goal.done ? '검증 완료' : '진행 중'}</em></div>)}</div> : <Empty>이번 주 능력 목표가 없습니다.<br />Weekly에서 성공 기준을 설정하세요.</Empty>}</Card></div>
  </div>;
}
