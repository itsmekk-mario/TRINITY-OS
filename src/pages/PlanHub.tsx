...
export default function PlanHub({ data, update, view, onView }: { data: AppData; update: (fn: (value: AppData) => AppData) => void; view: PlanView; onView: (view: PlanView) => void }) {
  ...
  const outcome = (item: typeof plans[number]) => item.outcome ?? (item.done ? 'achieved' : 'partial');
  const goals = data.weeklyCapabilityGoals.filter((goal) => goal.weekStart === week);
  const upcomingResources = sortResourcesByDeadline(data.resources.filter(resource => resource.done < resource.total && resource.dueDate)).slice(0, 3);
  return layout(<div>
    <div className="hub-metric-grid"><Card><CalendarDays /><span>오늘 일정</span><strong className="metric-number">{todayPlans.filter((item) => outcome(item)==='achieved').length}/{todayPlans.length}</strong><Progress value={todayPlans.filter((item) => outcome(item)==='achieved').length} max={Math.max(1, todayPlans.length)} /></Card><Card><CalendarRange /><span>이번 주 결과</span><strong className="metric-number">{plans.length ? Math.round(plans.filter((item) => outcome(item)==='achieved').length / plans.length * 100) : 0}%</strong><small>{plans.length ? `달성 ${plans.filter(item=>outcome(item)==='achieved').length} · 세모 ${plans.filter(item=>outcome(item)==='partial').length} · 실패 ${plans.filter(item=>outcome(item)==='failed').length}` : '주간 계획 없음'}</small></Card><Card><CheckCircle2 /><span>Capability Goal</span><strong className="metric-number">{goals.filter((item) => item.done).length}/{goals.length}</strong><small>성공 기준으로 검증</small></Card></div>
    ...
    {upcomingResources.length > 0 && <Card className="plan-upcoming-resources"><div className="hub-card-head"><div><span className="card-label">UPCOMING RESOURCES</span><h2>가까운 자료 마감</h2></div></div><div className="compact-list">{upcomingResources.map(resource => <div key={resource.id}><ListTodo /><span><b>{resource.name}</b><small>{resource.done} / {resource.total}</small></span><em>{formatResourceDeadline(resource)}</em></div>)}</div></Card>}
  </div>);
}
