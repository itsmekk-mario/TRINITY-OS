...
export default function Statistics({ data }: { data: AppData }) {
  ...
  const sessionsByDate = useMemo(() => studyTotals(data.sessions), [data.sessions]);
  ...
  const monthSubjectTotals = Object.fromEntries(SUBJECTS.map((subject) => [subject, Object.entries(studyTotals(data.sessions.filter(s => s.subject === subject))).filter(([date]) => date.startsWith(selectedMonth)).reduce((sum, [,seconds]) => sum + seconds, 0)])) as Record<Subject, number>;
  const subjectTotal = Object.values(monthSubjectTotals).reduce((sum, seconds) => sum + seconds, 0);
  const monthTrend = Array.from({ length: 6 }, (_, reverseIndex) => { const date = new Date(monthDate.getFullYear(), monthDate.getMonth() - (5 - reverseIndex), 1); const key = monthKey(date); return { key, label: `${date.getMonth() + 1}월`, seconds: Object.entries(sessionsByDate).filter(([date]) => date.startsWith(key)).reduce((sum, [,seconds]) => sum + seconds, 0) }; });
