...
function MonthHeatmap({ month, totals }: { month: Date; totals: Record<string, number> }) {
  ...
  return <section className="insight-month"><header><b>{month.getMonth() + 1}월</b><small>{compact(total)}</small></header><div className="insight-weekdays">{DAY.map(d => <span key={d}>{d}</span>)}</div><div className="insight-calendar">{Array.from({ length: first.getDay() }, (_, i) => <i key={i} />)}{dates.map((date, i) => { const seconds = totals[date] ?? 0; return <div key={date} title={`${date} · ${compact(seconds)}`} className={seconds ? 'has-study' : ''} style={seconds ? { '--level': String(.13 + Math.min(.87, seconds / max * .87)) } as React.CSSProperties : undefined}>{i + 1}</div>; })}</div></section>;
}
...
export default function Statistics({ data }: { data: AppData }) {
  ...
  const timed = data.sessions.filter((s): s is TimerSession & { startedAt: string; endedAt: string } => !!s.startedAt && !!s.endedAt && Number.isFinite(Date.parse(s.startedAt)) && Number.isFinite(Date.parse(s.endedAt)));
  ...
  const monthCount = Math.min(12, Math.max(1, Math.ceil(range / 30) + 1)); const months = Array.from({ length: monthCount }, (_, i) => new Date(end.getFullYear(), end.getMonth() - (monthCount - 1 - i), 1));
  ...
  return <div className="insight-page"><PageHeader eyebrow="STATISTICS" title="통계" description="순공 시간의 흐름과 과목 배분을 간결하게 확인합니다." />
    <div className="insight-tabs" role="tablist">{([{ label: '최근 30일', value: 30 }, { label: '3개월', value: 90 }, { label: '6개월', value: 180 }, { label: '최근 1년', value: 365 }] as const).map(item => <button key={item.value} className={range === item.value ? 'active' : ''} onClick={() => setRange(item.value)}>{item.label}</button>)}</div>
    ...
    <div className="insight-layout"><Card className="insight-calendar-card"><SectionTitle title="학습량 캘린더" meta={`${active.length}일 학습`} /><div className="insight-months">{months.map(month => <MonthHeatmap key={monthKey(month)} month={month} totals={totals} />)}</div></Card><div className="insight-right"><Card><SectionTitle title="과목 비율" meta={compact(total)} />{total ? <div className="insight-subjects"><div className="insight-donut" style={{ background: `conic-gradient(${gradient})` }}><b>{Math.round(total / 3600)}<small>시간</small></b></div><div>{SUBJECTS.map(subject => <p key={subject}><i style={{ background: COLOR[subject] }} /><span>{subject}</span><b>{compact(subjects[subject])}</b><small>{Math.round(subjects[subject] / total * 100)}%</small></p>)}</div></div> : <Empty>선택한 기간의 타이머 기록이 없습니다.</Empty>}</Card><Card><SectionTitle title="일별 과목 시간" meta="최근 28일" /><DailyBars dates={latest} totals={totals} byDay={byDay} /></Card><Card><SectionTitle title="일별 시작·종료" meta="시각이 있는 타이머 기록만 반영" /><div className="insight-time-points"><div><Clock3 /><span>평균 시작</span><b>{timeText(averageTime(startMinutes))}</b></div><div><Timer /><span>평균 종료</span><b>{timeText(averageTime(endMinutes))}</b></div><div><TrendingUp /><span>기록 세션</span><b>{timed.length}회</b></div></div></Card></div></div>
