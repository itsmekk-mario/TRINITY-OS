...
export default function Journal({ data, update }: { data: AppData; update: (fn: (value: AppData) => AppData) => void }) {
  ...
  return <div><PageHeader eyebrow="METACOGNITION JOURNAL" title="행동 수정 저널" description="감상을 기록하지 않습니다. 관찰한 현상을 원인·가설·다음 행동으로 변환합니다." action={<input className="date-input" type="date" value={date} onChange={(e) => setDate(e.target.value)} />} />
