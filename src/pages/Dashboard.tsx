...
import { ArrowRight, CalendarDays, Check, Clock3, Crosshair, MessageSquareText, Sparkles } from 'lucide-react';
...
export default function Dashboard({ data, update, navigate }: { data: AppData; update: (fn: (value: AppData) => AppData) => void; navigate: (page: string) => void }) {
  ...
  const examCountdown = useMemo(() => {
    ...
    const target=new Date(`${data.examDate}T00:00:00`),base=new Date(now.getFullYear(),now.getMonth(),now.getDate());
    if(Number.isNaN(target.getTime()))return null;
    return Math.round((target.getTime()-base.getTime())/86_400_000);
  },[data.examDate,today]);
  const countdownLabel=examCountdown===null?'날짜 설정':examCountdown===0?'D-DAY':examCountdown>0?`D-${examCountdown}`:`D+${Math.abs(examCountdown)}`;
  ...
  return <div className="today-page"><PageHeader eyebrow="TODAY" title="오늘의 학습" description={formatKoreanDate(now)} />
    <section className="today-hero" aria-label="오늘 핵심 현황"><div className="today-primary"><span>오늘 공부시간</span><strong className="metric-number">{formatMinutes(analytics.execution.todayMinutes)}</strong><small>{analytics.execution.plannedMinutes ? `계획 ${formatMinutes(analytics.execution.plannedMinutes)}` : '시간 계획이 아직 없습니다.'}</small></div><div className="today-primary"><span>계획 대비 실행</span><strong className="metric-number">{analytics.execution.completionRate ?? 0}%</strong><small>{analytics.execution.planItemRate === null ? '오늘 계획 없음' : `항목 완료율 ${analytics.execution.planItemRate}%`}</small></div><div className="today-primary today-countdown"><span><CalendarDays size={14}/> 수능까지</span><strong className="metric-number">{countdownLabel}</strong><small>{data.examDate ? new Date(`${data.examDate}T00:00:00`).toLocaleDateString('ko-KR',{year:'numeric',month:'long',day:'numeric'}) : 'Settings에서 수능 날짜를 설정하세요.'}</small></div><div className="today-next"><span>NEXT</span><strong>{next?.title ?? '오늘 계획 완료'}</strong><small>{next ? `${next.subject} · ${next.quantity || '목표량 미설정'}` : '다음 학습을 계획해도 좋습니다.'}</small><button onClick={() => navigate(next ? 'train:timer' : 'plan:calendar')}>{next ? 'Timer 시작' : '계획 추가'} <ArrowRight size={16} /></button></div></section>
