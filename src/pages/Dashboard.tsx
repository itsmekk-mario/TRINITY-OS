...
import { ArrowRight, Check, Crosshair, MessageSquareText, RotateCcw } from 'lucide-react';
...
import type { ActiveCoreRule, ArchiveSubject } from '../lib/archiveApi.ts';
...
export default function Dashboard({ data, update, navigate }: { data: AppData; update: (fn: (value: AppData) => AppData) => void; navigate: (page: string) => void }) {
  ...
  const [reviewDue,setReviewDue]=useState<{overdue:number;today:number;due:number}>({overdue:0,today:0,due:0});
  useEffect(()=>{let active=true;void import('../lib/archiveApi.ts').then(({archiveApi})=>archiveApi<{counts:{overdue:number;today:number;due:number}}>('/api/learning-intelligence/reviews?view=queue')).then(value=>{if(active)setReviewDue(value.counts)}).catch(()=>{if(active)setReviewDue({overdue:0,today:0,due:0})});return()=>{active=false}},[]);
