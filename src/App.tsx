...
function StudentApp() {
  ...
  useEffect(() => {
    const retry = () => {
      if (identity) setData((value) => ({ ...value }));
    };
  ...
  useEffect(() => {
    ...
    void validateSession().then((profile) => {
      if (!profile) {
        logoutLocal();
        setAuthenticated(false);
        return;
      }
      setPasswordRequired(Boolean(profile.mustChangePassword));
      setData(loadData(profile.userId));
      setIdentity(profile);
    });
  }, [authenticated]);
  useEffect(() => {
  ...
  const navigate = (targetValue: string) => {
    ...
    const nextKey = view ? `${target}:${view}` : target;
    const url = `${paths[target]}${view ? `?view=${encodeURIComponent(view)}` : ""}`;
    const commit = () => {
      ...
      window.requestAnimationFrame(() =>
        window.scrollTo({
          top: scrollPositions.current.get(nextKey) ?? 0,
          behavior: "auto",
        }),
      );
    };
  ...
  const changeSubview = <T extends string>(
    target: Page,
    view: T,
    setter: (value: T) => void,
  ) => {
    ...
    window.history.pushState(
      {},
      "",
      `${paths[target]}?view=${encodeURIComponent(view)}`,
    );
    window.requestAnimationFrame(() =>
      window.scrollTo({
        top: scrollPositions.current.get(`${target}:${view}`) ?? 0,
        behavior: "auto",
      }),
    );
  };
  const importData = async (file?: File) => {
    if (!file) return;
    try {
      setBackupPreview(await parseBackup(file));
    } catch (error) {
      setToast(
  ...
  const confirmImport = async () => {
    ...
    try {
      const version = backupPreview.version;
      setData(await restoreBackup(backupPreview));
      setBackupPreview(null);
      setToast(
        version === 2
          ? "App Data와 Learning Archive를 복원했습니다."
          : "App Data를 복원했습니다.",
      );
      setSettings(false);
    } catch (error) {
      setToast(error instanceof Error ? error.message : "복원에 실패했습니다.");
    } finally {
      window.setTimeout(() => setToast(""), 3000);
    }
  };
  const exportAll = async () => {
    try {
      ...
      setToast("전체 백업을 다운로드했습니다.");
    } catch (error) {
      setToast(
        error instanceof Error
          ? `전체 백업 실패: ${error.message}`
          : "전체 백업에 실패했습니다.",
      );
    } finally {
      window.setTimeout(() => setToast(""), 3000);
  ...
  const screen =
    page === "today" ? (
      ...
      <PlanHub
        ...
        update={update}
        view={planView}
        onView={(view) => changeSubview("plan", view, setPlanView)}
      />
    ) : page === "train" ? (
      <TrainHub
        data={data}
      ...
      <FeedbackInbox data={data} update={update} />
    ) : page === "workspace" ? (
      <NotionWorkspace data={data} update={update} />
    ) : page === "profile" ? (
      <Arena data={data} />
    ) : (
      <ExamArchive />
    ...
    return (
      <LoginPage
        onAuthenticated={() => {
          ...
          setPasswordRequired(Boolean(config.mustChangePassword));
        }}
      />
    );
  if (!identity)
  ...
  return (
    <div className="app-shell learning-shell">
      <aside
        ...
        aria-label={menu ? "메뉴" : undefined}
        className={menu ? "open" : ""}
      >
        <div className="brand">
          <div className="brand-mark">T</div>
          <div>
            <strong>TRINITY OS</strong>
      ...
      <main>
        <div className="mobile-bar">
          ...
          <button aria-label="설정 열기" onClick={() => setSettings(true)}>
            <Settings />
          </button>
        </div>
      ...
      {settings && (
        ...
        >
          <aside
            ref={settingsRef}
            tabIndex={-1}
            className="detail-sheet settings-sheet"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header>
              <div>
                ...
                <h2 id="settings-title">데이터 및 설정</h2>
              </div>
              <button
            ...
            <div className="sheet-content">
              <section
                ...
                aria-labelledby="theme-setting-title"
              >
                <div>
                  <b id="theme-setting-title">화면 테마</b>
                  <p>
                    시스템은 기기 설정을 따르고, 라이트·다크는 직접 고정합니다.
                  </p>
                </div>
                <div
          ...
          <section
            className="theme-setting beginner-mode-setting"
            aria-labelledby="beginner-mode-title"
          >
            <div>
              <b id="beginner-mode-title">초보자용 화면</b>
              <p>
                핵심 학습 메뉴만 남겨 화면을 단순하게 표시합니다. 기존 데이터와 기록은 그대로 유지됩니다.
              </p>
            </div>
            <button
              type="button"
              className={`beginner-mode-toggle ${beginnerMode ? 'active' : ''}`}
              aria-pressed={beginnerMode}
              onClick={() => changeBeginnerMode(!beginnerMode)}
            >
              ...
              <div className="inline-settings">
                ...
                <label>
                  ...
                  <input
                    ...
                    value={data.examDate}
                    onChange={(event) =>
                      setData({ ...data, examDate: event.target.value })
                    }
                  />
                </label>
                <label>
                  <span>오늘의 문장 · 한 줄에 하나</span>
                  <textarea
                    rows={5}
                    ...
                    onChange={(event) =>
                      setData({
                        ...data,
                        quotes: event.target.value.split("\n").filter(Boolean),
                      })
                    }
                  />
                </label>
              ...
              <div className="settings-list">
                ...
                >
                  ...
                  <span>
                    <b>비밀번호 변경</b>
                    <small>현재 비밀번호를 확인하고 변경합니다.</small>
                  </span>
                </button>
                <button onClick={exportAll}>
                  ...
                  <span>
                    <b>전체 백업 다운로드</b>
                    <small>
                      Backup v2 · Calendar, Sessions, Scores, Wrong Answer,
                      Learning Archive, Core Rules, Review History 포함
                    </small>
                  </span>
                </button>
              ...
              {backupPreview && (
                <section className="backup-preview">
                  <b>복원 미리보기 · Backup v{backupPreview.version}</b>
                  <p>
                    Calendar {Object.keys(backupPreview.app.calendar).length} ·
                    Sessions {backupPreview.app.sessions.length} · Scores{" "}
                    {backupPreview.app.scores.length} · Wrong Answer{" "}
                    {backupPreview.app.wrongAnswerDrills.length}
                  </p>
                  {backupPreview.learningArchive ? (
                    <p>
                      Learning Archive: Entries{" "}
                      {backupPreview.learningArchive.entries.length} ·
                      Annotations{" "}
                      {backupPreview.learningArchive.annotations.length} · Core
                      Rules {backupPreview.learningArchive.coreRules.length} ·
                      Rule Links{" "}
                      {backupPreview.learningArchive.ruleLinks.length} · Reviews{" "}
                      {backupPreview.learningArchive.reviews.length}
                    </p>
                  ) : (
                    <p>Learning Archive 데이터는 포함되어 있지 않습니다.</p>
                  )}
              ...
              <div className="privacy-note">
                ...
                <div>
                  <b>계정별 전체 백업</b>
                  <p>
                    App Data는 기기에, Learning Archive는 로그인한 학생 계정의
                    D1 데이터에 복원됩니다.
                  </p>
                </div>
              </div>
            </div>
            <footer>TRINITY OS v{APP_VERSION}</footer>
          </aside>
        </div>
      )}
      {(passwordRequired || passwordDialog) && (
...
export default function App() {
  ...
  if (portal === "teacher" || portal === "tutor")
    return (
      <Suspense
        fallback={
          <main className="team-page" role="status">
            교사 포털을 불러오는 중…
          </main>
        }
      >
        <CollaborativePortal role="subject_teacher" />
      </Suspense>
    );
  if (portal === "english-teacher")
    return (
      <Suspense
        fallback={
          <main className="team-page" role="status">
      ...
      >
        <CollaborativePortal role="subject_teacher" subjectHint="영어" />
      </Suspense>
    );
  if (portal === "manager")
    return (
      ...
      >
        <CollaborativePortal role="academic_manager" />
      </Suspense>
    );
