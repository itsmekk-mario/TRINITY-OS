...
function StudentApp() {
  const [page,setPage]=useState(() => window.location.pathname === '/arena' ? 'arena' : 'dashboard'); const [data,setData]=useState<AppData>(loadData); const [menu,setMenu]=useState(false); const [settings,setSettings]=useState(false); const [passwordDialog,setPasswordDialog]=useState(false); const [passwordRequired,setPasswordRequired]=useState(Boolean(loadCloudflareConfig().mustChangePassword)); const [toast,setToast]=useState(''); const fileRef=useRef<HTMLInputElement>(null); const [authenticated,setAuthenticated]=useState(Boolean(loadCloudflareConfig().token)); const syncing=useRef(false);
