...
function StudentApp() {
  ...
  useEffect(() => {
    ...
    const apply = () => { const theme = resolveTheme(themePreference); document.documentElement.dataset.theme = theme; document.documentElement.style.colorScheme = theme; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme === 'dark' ? '#111216' : '#f5f5f7'); };
    apply();
    const onChange = () => { if (themePreference === 'system') apply(); };
  ...
  const changeTheme = (next: ThemePreference) => { localStorage.setItem(THEME_KEY, next); setThemePreference(next); };
