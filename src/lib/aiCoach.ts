...
export type CoachReply = { summary: string; bottleneck: string; nextAction: string; coachMessage: string };
...
export function buildDailyCoachContext(data: AppData, now = new Date()): DailyCoachContext {
  ...
  const todaySchedule = [
    ...plans.map((item: CalendarPlan) => ({ subject: item.subject, title: item.title, plannedMinutes: minutesFrom(item.quantity), completed: item.done })),
    ...todayDrills.map((item) => ({ subject: item.subject, title: item.title, plannedMinutes: item.minutes, completed: item.done })),
  ];
  ...
  const subjectProgress = subjects.map((subject) => ({
    subject,
    plannedMinutes: todaySchedule.filter((item) => item.subject === subject).reduce((sum, item) => sum + (item.plannedMinutes ?? 0), 0),
    completedMinutes: Math.round(data.sessions.filter((item) => item.date === date && item.subject === subject).reduce((sum, item) => sum + item.seconds, 0) / 60),
  })).filter((item) => item.plannedMinutes || item.completedMinutes);
  ...
  const recentBottlenecks = [...new Set(data.wrongAnswerDrills.filter((item) => item.date >= recentKey).map((item) => item.bottleneck).filter(Boolean).map((item) => String(item)))].slice(0, 3);
...
export function fallbackCoachMessage(context: DailyCoachContext) {
  const pending = context.todaySchedule.find((item) => !item.completed);
  return pending ? `남은 ${pending.title}부터 차례대로 완료하세요.` : '오늘 기록을 짧게 정리하고, 다음 학습의 기준 한 가지를 남기세요.';
}

const cacheKey = (context: DailyCoachContext) => `trinity-os:daily-coach:${context.date}:${JSON.stringify(context).length}:${context.progress.completedMinutes}:${context.todaySchedule.filter((item) => item.completed).length}`;
export function loadCoachCache(context: DailyCoachContext): CoachReply | null { try { const raw = localStorage.getItem(cacheKey(context)); const cached = raw ? JSON.parse(raw) as { expiresAt: number; value: CoachReply } : null; return cached && cached.expiresAt > Date.now() ? cached.value : null; } catch { return null; } }
export function saveCoachCache(context: DailyCoachContext, value: CoachReply) { localStorage.setItem(cacheKey(context), JSON.stringify({ value, expiresAt: Date.now() + 30 * 60_000 })); }

async function request<T>(path: string, body: unknown): Promise<T> {
  const config = loadCloudflareConfig(); if (!config.url || !config.token) throw new Error('AI 코치를 사용하려면 Cloudflare 동기화 로그인이 필요합니다.');
  const response = await fetch(`${config.url.replace(/\/+$/, '')}${path}`, { method: 'POST', headers: { Authorization: `Bearer ${config.token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  const payload = await response.json() as T & { error?: string }; if (!response.ok) throw new Error(payload.error || 'AI 코치 연결에 실패했습니다.'); return payload;
}
export const requestDailyCoach = (context: DailyCoachContext) => request<CoachReply>('/api/ai/daily-coach', { context });
