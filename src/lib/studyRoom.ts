...
export type MediaServerStatus = { online: boolean; status: 'online' | 'offline' | 'not_configured' };
...
async function request<T>(
  ...
  if (!response.ok) {
    throw new Error(value.error || (response.status === 409
...
export const createStudyRoom = (name: string, maxParticipants: number) =>
  request<{ room: StudyRoomInfo }>('/api/study-rooms', { name, maxParticipants });

export const joinStudyRoom = (code: string) =>
  request<{ room: StudyRoomInfo }>('/api/study-rooms/join', { code });

export const createStudyRoomTicket = (code: string) => request<{
  ticket: string;
  expiresAt: number;
...
export const createLiveKitAccess = (code: string, connectionId: string) =>
  request<LiveKitAccess>(`/api/study-rooms/${encodeURIComponent(code)}/livekit/token`, { connectionId });

export const getMediaServerStatus = () =>
  request<MediaServerStatus>('/api/study-rooms/media-status');
