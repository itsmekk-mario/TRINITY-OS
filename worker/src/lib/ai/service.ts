import { NvidiaNimProvider, type NvidiaNimConfig } from './providers/nvidia-nim.ts';
...
export interface AIServiceConfig extends NvidiaNimConfig { provider?: string; }
...
export class AIService {
  ...
  private provider(config: AIServiceConfig): AIProvider {
    ...
    if (!config.provider || config.provider === 'nvidia-nim' || config.provider === 'nvidia-kimi') return new NvidiaNimProvider(config);
