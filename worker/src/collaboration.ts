...
export const outData = (data: AppData, assignment: Assignment) => {
  ...
  const plaire = !parent && manager ? Object.values(data.plaire??{}).slice(-7).map(({date,bottleneck,nextAction})=>({date,bottleneck,nextAction})) : [];
  ...
  return { access,sessions,scores,plans,wrongAnswerDrills,weeklyCapabilityGoals,dailyDrills,resources,plaire,trinity,monthlyPlans,routine,goals };
};
...
export async function collaboration(request: Request, env: Env, owner: boolean, account: Account | null, origin: string, h: Helpers, studentActor: StudentActor = null): Promise<Response | null> {
  ...
  try {
    ...
    if(match[2]==='data'&&method==='GET'){
      if(!data)return out({studentId:student.arena_public_id,data:null,teacherView:null}); const view=outData(data,assignment); const today=new Date().toLocaleDateString('sv-SE',{timeZone:'Asia/Seoul'}); const monday=new Date(today+'T12:00:00Z'); monday.setUTCDate(monday.getUTCDate()-((monday.getUTCDay()||7)-1)); const weekStart=monday.toISOString().slice(0,10); const weeklySessions=view.sessions.filter(item=>item.date>=weekStart&&item.date<=today); const weeklyPlans=view.plans.filter(item=>item.date>=weekStart&&item.date<=today); const teacherView={student:{id:student.arena_public_id,displayName:student.username},progress:{weeklyStudyMinutes:view.access.sessions?Math.round(weeklySessions.reduce((total,item)=>total+item.seconds,0)/60):null,executionRate:weeklyPlans.length?Math.round(weeklyPlans.filter(item=>item.done).length/weeklyPlans.length*100):null,subjectProgress:weeklySessions.reduce<Record<string,number>>((totals,item)=>{totals[item.subject]=(totals[item.subject]??0)+item.seconds;return totals;},{})},sessions:view.sessions,mockExams:view.scores,scores:view.scores,wrongAnswerDrills:view.wrongAnswerDrills,weeklyGoals:view.weeklyCapabilityGoals,dailyDrills:view.dailyDrills,resources:view.resources,academicInsights:view.plaire}; return out({studentId:student.arena_public_id,data:{sessions:view.sessions,scores:view.scores,wrongAnswerDrills:view.wrongAnswerDrills,weeklyCapabilityGoals:view.weeklyCapabilityGoals,dailyDrills:view.dailyDrills,resources:view.resources,plaire:view.plaire,plans:view.plans,trinity:view.trinity,access:view.access},syncedAt:(await env.DB.prepare('SELECT updated_at FROM learning_state WHERE user_id=?').bind(student.id).first<{updated_at:string}>())?.updated_at,teacherView});
    }
