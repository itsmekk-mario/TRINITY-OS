...
export async function collaboration(request: Request, env: Env, owner: boolean, account: Account | null, origin: string, h: Helpers): Promise<Response | null> {
  ...
  try {
    ...
    if (edit && method === 'PATCH') {
      ...
      await env.DB.prepare('UPDATE teacher_feedback SET title=?,categories_json=?,status=?,progress=?,bottleneck=?,observation=?,action=?,success_criterion=?,comment=?,updated_at=? WHERE id=?').bind(clean(body.title,160),JSON.stringify(asArray(body.categories)),status,progress,clean(body.bottleneck),clean(body.observation,2000),clean(body.action,1200),clean(body.successCriterion,1200),clean(body.comment,2000),now,edit[1]).run();
