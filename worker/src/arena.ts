...
type ArenaTools = { json: JsonResponder; randomHex: (size?: number) => string };
...
type RankingRow = { user_id: number; nickname: string; target_university: string; target_department: string; score: number; growth_rate: number };
async function ranking(db: D1Database, userId: number, seasonId: string, groupId = '') {
  ...
  const statement = db.prepare(`SELECT p.user_id,p.nickname,p.target_university,p.target_department,s.score,s.growth_rate FROM arena_profiles p JOIN arena_score_snapshots s ON s.user_id=p.user_id WHERE s.season_id=? AND s.calculated_at=(SELECT MAX(s2.calculated_at) FROM arena_score_snapshots s2 WHERE s2.user_id=s.user_id AND s2.season_id=s.season_id) ${membership} ORDER BY s.score DESC,s.growth_rate DESC,s.calculated_at ASC LIMIT 100`);
  const result = groupId ? await statement.bind(seasonId, groupId).all<RankingRow>() : await statement.bind(seasonId).all<RankingRow>();
  return result.results.map((row, index) => ({ rank: index + 1, userId: row.user_id, nickname: row.nickname, score: row.score, growthRate: row.growth_rate, targetUniversity: row.target_university, targetDepartment: row.target_department, isMe: row.user_id === userId }));
}
...
async function rivals(db: D1Database, userId: number, seasonId: string) {
  const rows = await db.prepare(`SELECT p.user_id,p.nickname,p.target_university,p.target_department,s.score,s.growth_rate,s.metrics FROM arena_rivals r JOIN arena_profiles p ON p.user_id=r.rival_user_id LEFT JOIN arena_score_snapshots s ON s.user_id=p.user_id AND s.season_id=? AND s.calculated_at=(SELECT MAX(s2.calculated_at) FROM arena_score_snapshots s2 WHERE s2.user_id=p.user_id AND s2.season_id=?) WHERE r.user_id=? ORDER BY r.created_at DESC`).bind(seasonId, seasonId, userId).all<RankingRow & { metrics: string | null }>();
  ...
  ]; const gap = comparison.map(item => ({ ...item, delta: item.mine - item.rival })).sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta))[0]; return { rank: index + 1, userId: row.user_id, nickname: row.nickname, score: row.score || 0, growthRate: row.growth_rate || 0, targetUniversity: row.target_university, targetDepartment: row.target_department, comparison, insight: gap ? `${gap.label}에서 ${Math.abs(gap.delta).toFixed(1)}${gap.unit} 차이가 가장 큽니다. 승패보다 다음 주 변화 폭을 확인하세요.` : '비교할 학습 기록이 아직 부족합니다.' }; });
}
...
export async function arena(request: Request, env: ArenaEnv, user: ArenaUser | null, origin: string, tools: ArenaTools): Promise<Response | null> {
  ...
  if (url.pathname === '/api/arena/profile' && request.method === 'PUT') {
    const body = object(await request.json<unknown>()); if (!body) return tools.json({ error: '프로필 정보가 필요합니다.' }, 400, origin);
  ...
  if (url.pathname === '/api/arena/score' && request.method === 'POST') {
    const body = object(await request.json<unknown>()); const breakdown = object(body?.breakdown); const metrics = object(body?.metrics);
    if (!body || !breakdown || !metrics) return tools.json({ error: '점수 산출 정보가 필요합니다.' }, 400, origin);
    const execution = Math.round(number(breakdown.execution, 0, 300)), problem = Math.round(number(breakdown.problemSolving, 0, 300)), consistency = Math.round(number(breakdown.consistency, 0, 200)), growth = Math.round(number(breakdown.growth, 0, 200)); const total = execution + problem + consistency + growth;
    const calculatedAt = string(body.calculatedAt, 40) || now; const growthRate = number(metrics.growthRate, -99, 200);
    ...
    return tools.json({ ok: true, achievements: await achievements(env.DB, user.id) }, 200, origin);
  }

  if (url.pathname === '/api/arena/groups' && request.method === 'POST') {
    const body = object(await request.json<unknown>()); const name = string(body?.name, 50); const type = string(body?.type, 20); const visibility = string(body?.visibility, 20);
  ...
  if (url.pathname === '/api/arena/groups/join' && request.method === 'POST') {
    const body = object(await request.json<unknown>()); const lookup = string(body?.groupIdOrCode, 80); if (!lookup) return tools.json({ error: '그룹 또는 초대 코드를 입력해 주세요.' }, 400, origin);
  ...
  if (url.pathname === '/api/arena/ranking' && request.method === 'GET') return tools.json({ ranking: await ranking(env.DB, user.id, currentSeason.id, string(url.searchParams.get('group'), 80)) }, 200, origin);

  if (url.pathname === '/api/arena/rivals' && request.method === 'POST') {
    const body = object(await request.json<unknown>()); const rivalId = Math.floor(number(body?.rivalUserId, 0, Number.MAX_SAFE_INTEGER)); if (!rivalId || rivalId === user.id) return tools.json({ error: '라이벌을 확인해 주세요.' }, 400, origin);
  ...
  if (url.pathname.startsWith('/api/arena/rivals/') && request.method === 'DELETE') { const rivalId = Number(url.pathname.slice('/api/arena/rivals/'.length)); await env.DB.prepare('DELETE FROM arena_rivals WHERE user_id=? AND rival_user_id=?').bind(user.id, rivalId).run(); return tools.json({ ok: true }, 200, origin); }
