...
export async function getCoreRuleStatsBatch(db:D1Database,userId:number,coreRuleIds:string[]):Promise<Map<string,CoreRuleStats>>{
  const ids=[...new Set(coreRuleIds.filter(Boolean))],result=new Map<string,CoreRuleStats>();
  const empty=():CoreRuleStats=>({evidenceCount:0,archiveCount:0,wrongAnswerCount:0,drillCount:0,derivedCount:0,appliedCount:0,failedCount:0,reinforcedCount:0,failures7d:0,failures30d:0,lastOccurrenceAt:null,lastFailureAt:null,reviewCount:0,reviewSuccessCount:0,reviewFailureCount:0,masteryRate:null});
  for(const id of ids)result.set(id,empty());
  if(!ids.length)return result;
  const cut7=new Date(Date.now()-7*86400000).toISOString(),cut30=new Date(Date.now()-30*86400000).toISOString();
  const placeholders=ids.map(()=>'?').join(',');
  const rows=await db.prepare(`SELECT core_rule_id,
    COUNT(*) evidence_count,
    COUNT(DISTINCT CASE WHEN source_type='archive' THEN source_id END) archive_count,
    COUNT(DISTINCT CASE WHEN source_type='wrong_answer' THEN source_id END) wrong_answer_count,
    COUNT(DISTINCT CASE WHEN source_type='drill' THEN source_id END) drill_count,
    ...
    COUNT(DISTINCT CASE WHEN source_type='review' AND relation_type='reinforced' THEN source_id END) review_success_count,
    COUNT(DISTINCT CASE WHEN source_type='review' AND relation_type='failed' THEN source_id END) review_failure_count
    FROM core_rule_evidence WHERE user_id=? AND core_rule_id IN (${placeholders}) GROUP BY core_rule_id`)
    .bind(cut7,cut30,userId,...ids).all<Record<string,unknown>>();
  for(const row of rows.results){
    const reviewCount=Number(row.review_count??0),reviewSuccessCount=Number(row.review_success_count??0),id=String(row.core_rule_id);
    result.set(id,{
      evidenceCount:Number(row.evidence_count??0),archiveCount:Number(row.archive_count??0),wrongAnswerCount:Number(row.wrong_answer_count??0),drillCount:Number(row.drill_count??0),
      derivedCount:Number(row.derived_count??0),appliedCount:Number(row.applied_count??0),failedCount:Number(row.failed_count??0),reinforcedCount:Number(row.reinforced_count??0),
      failures7d:Number(row.failures_7d??0),failures30d:Number(row.failures_30d??0),lastOccurrenceAt:row.last_occurrence_at?String(row.last_occurrence_at):null,
      lastFailureAt:row.last_failure_at?String(row.last_failure_at):null,reviewCount,reviewSuccessCount,reviewFailureCount:Number(row.review_failure_count??0),
      masteryRate:reviewCount?reviewSuccessCount/reviewCount:null,
    });
  }
  return result;
...
export async function listActiveCoreRules(db:D1Database,userId:number,subject:string|undefined,limit:number){
  ...
  const statsByRule=await getCoreRuleStatsBatch(db,userId,rows.results.map(rule=>String(rule.id)));
  const enriched=rows.results.map(rule=>{
    const stats=statsByRule.get(String(rule.id))!;
    const priority=calculateCoreRulePriority(stats,String(rule.mastery_status??'input'));
    return {id:String(rule.id),title:String(rule.title??''),subject:String(rule.subject??''),content:String(rule.content??''),tags:parse<string[]>(rule.tags_json,[]),masteryStatus:String(rule.mastery_status??'input'),...priority,stats};
  });
