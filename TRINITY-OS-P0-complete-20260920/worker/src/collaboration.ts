...
async function validateFeedbackLinks(db:D1Database,studentId:number,assignment:Assignment,data:AppData|null,links:ReturnType<typeof feedbackLinks>){
  const view=data?outData(data,assignment):null;
  for(const link of links){
    if(link.type==='core_rule'||link.type==='learning_item'){
      const table=link.type==='core_rule'?'core_rules':'archive_entries',row=await db.prepare(`SELECT subject FROM ${table} WHERE id=? AND user_id=?`).bind(link.id,studentId).first<{subject:string}>();
      if(!row||assignment.role==='subject_teacher'&&row.subject!==archiveSubjectFor(assignment.subject))throw new Error('Invalid feedback relation');
    }else if(link.type==='wrong_answer'||link.type==='drill'||link.type==='mock_exam'){
      const items=link.type==='wrong_answer'?view?.wrongAnswerDrills:link.type==='drill'?view?.dailyDrills:view?.scores;
      if(!items?.some(item=>item.id===link.id))throw new Error('Invalid feedback relation');
    }else if(link.type==='subject_progress'&&(!view||!view.sessions.some(item=>item.subject===link.id)||assignment.role==='subject_teacher'&&link.id!==assignment.subject))throw new Error('Invalid feedback relation');
  }
}
...
export async function collaboration(request: Request, env: Env, owner: boolean, account: Account | null, origin: string, h: Helpers, studentActor: StudentActor = null): Promise<Response | null> {
  ...
  try {
    ...
    const rows = await env.DB.prepare("SELECT f.*,a.username AS teacher_name,a.collaboration_role AS teacher_role FROM teacher_feedback f JOIN support_accounts a ON a.id=f.teacher_id WHERE f.student_user_id=? AND f.deleted_at IS NULL ORDER BY CASE f.progress WHEN 'active' THEN 0 ELSE 1 END,f.created_at DESC LIMIT 100").bind(studentActor.id).all<Record<string,unknown>>();
    ...
    if(edit&&method==='PATCH'){
      const previous=await env.DB.prepare('SELECT * FROM teacher_feedback WHERE id=? AND teacher_id=? AND deleted_at IS NULL').bind(edit[1],account.id).first<Record<string,unknown>>();if(!previous)return out({error:'Feedback not found'},404);const authorAccess=await assignmentFor(env,account,String(previous.student_id));if(!authorAccess||!allowed(authorAccess.assignment,'createFeedback'))return out({error:'Feedback permission required'},403);const body=await h.boundedJson<Record<string,unknown>>(request),has=(key:string)=>Object.prototype.hasOwnProperty.call(body,key),pick=(key:string,column:string,max=1200)=>has(key)?clean(body[key],max):String(previous[column]??'');
      if(has('status')&&!['needs_improvement','normal','stable'].includes(String(body.status)))return out({error:'Invalid status'},400);if(has('progress')&&!['active','achieved','replaced','archived'].includes(String(body.progress)))return out({error:'Invalid progress'},400);if(has('categories')&&!Array.isArray(body.categories))return out({error:'Invalid categories'},400);
      const now=new Date().toISOString(),contextType=has('contextType')?feedbackContext(body).type:String(previous.context_type??'general'),contextTargetId=has('contextTargetId')?feedbackContext(body).targetId:previous.context_target_id??null;
      await env.DB.batch([env.DB.prepare('INSERT INTO teacher_feedback_audit(id,feedback_id,editor_id,snapshot_json,edited_at) VALUES(?,?,?,?,?)').bind(h.randomHex(16),edit[1],account.id,JSON.stringify(previous),now),env.DB.prepare('UPDATE teacher_feedback SET title=?,categories_json=?,status=?,progress=?,bottleneck=?,observation=?,action=?,success_criterion=?,comment=?,context_type=?,context_target_id=?,updated_at=? WHERE id=? AND teacher_id=? AND deleted_at IS NULL').bind(pick('title','title',160),has('categories')?JSON.stringify(asArray(body.categories)):String(previous.categories_json??'[]'),has('status')?String(body.status):String(previous.status),has('progress')?String(body.progress):String(previous.progress),pick('bottleneck','bottleneck'),pick('observation','observation',2000),pick('action','action',1200),pick('successCriterion','success_criterion',1200),pick('comment','comment',2000),contextType,contextTargetId,now,edit[1],account.id)]);return out({ok:true,updatedAt:now});
    }
    if(edit&&method==='PATCH'){
    ...
    if(edit&&method==='DELETE'){
      const previous=await env.DB.prepare('SELECT * FROM teacher_feedback WHERE id=? AND teacher_id=? AND deleted_at IS NULL').bind(edit[1],account.id).first<Record<string,unknown>>();if(!previous)return out({error:'Only the author can delete this feedback'},403);const authorAccess=await assignmentFor(env,account,String(previous.student_id));if(!authorAccess||!allowed(authorAccess.assignment,'createFeedback'))return out({error:'Feedback permission required'},403);const now=new Date().toISOString();await env.DB.batch([env.DB.prepare('INSERT INTO teacher_feedback_audit(id,feedback_id,editor_id,snapshot_json,edited_at) VALUES(?,?,?,?,?)').bind(h.randomHex(16),edit[1],account.id,JSON.stringify(previous),now),env.DB.prepare('UPDATE teacher_feedback SET deleted_at=?,deleted_by=?,updated_at=? WHERE id=? AND teacher_id=?').bind(now,account.id,now,edit[1],account.id)]);return out({ok:true,deletedAt:now});
    }
    ...
    if(match[2]==='feedback'&&method==='GET'){
      const rows=await env.DB.prepare('SELECT f.*,a.username AS teacher_name,a.collaboration_role AS teacher_role FROM teacher_feedback f JOIN support_accounts a ON a.id=f.teacher_id WHERE f.student_user_id=? AND f.deleted_at IS NULL ORDER BY f.created_at DESC LIMIT 100').bind(student.id).all<Record<string,unknown>>(); return out({feedback:rows.results.filter(row=>assignment.role==='academic_manager'||assignment.role==='admin'||assignment.role==='parent'||row.subject===assignment.subject||readSignal(row.signal_json)?.subject===assignment.subject).map(row=>({...feedbackRow(row),canEdit:row.teacher_id===account.id&&allowed(assignment,'createFeedback'),canDelete:row.teacher_id===account.id&&allowed(assignment,'createFeedback')}))});
    }
