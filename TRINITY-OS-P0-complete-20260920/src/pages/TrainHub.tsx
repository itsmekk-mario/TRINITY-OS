...
function WrongAnswers({ data, edit, quick }: { data: AppData; edit: () => void; quick: () => void }) {
  ...
  return <div><PageHeader eyebrow="WRONG ANSWERS" title="오답을 행동으로 바꿉니다" description="저장한 판단·놓친 단서·교정 행동을 한 번 입력하고 재도전까지 이어갑니다." action={<div className="split-actions"><button className="button" onClick={quick}>빠른 오답 기록</button><button className="button primary" onClick={edit}>오답 Drill 작성 <ArrowRight size={16} /></button></div>} />
    {items.length ? <><div className="wrong-view-toggle"><button className={view==='all'?'active':''} onClick={()=>setView('all')}>전체</button><button className={view==='subject'?'active':''} onClick={()=>setView('subject')}>과목별</button><button className={view==='source'?'active':''} onClick={()=>setView('source')}>출처별</button><button className={view==='time'?'active':''} onClick={()=>setView('time')}>시간별</button></div>{view==='all'?<div className="wrong-library">{items.map(card)}</div>:view==='subject'?renderGroups(subjectGroups):view==='source'?renderGroups(sourceGroups):renderGroups(timeGroups)}</> : <Empty title="아직 오답 Drill이 없습니다." description="틀린 판단을 기록하면 3/7/14일 재도전이 자동 생성됩니다." action={<div className="split-actions"><button className="button primary" onClick={quick}>빠른 오답 기록</button><button className="button" onClick={edit}>상세 오답 기록</button></div>} />}
