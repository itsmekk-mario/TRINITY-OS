...
test('Subject teacher uses the lightweight tutor workspace',()=>{
  assert.match(portal,/role\s*===\s*["']subject_teacher["']/);
...
test('Tutor archive subject mapping preserves math and never falls back to all subjects',()=>{
  ...
  assert.match(tutor,/filterTutorArchive[\s\S]*?archiveSubject[\s\S]*?archive\.filter[\s\S]*?:\s*\[\]/);
  assert.match(tutor,/filterTutorArchive\(archive,\s*subject\)/);
});
